...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (!empty($_POST['name']) && !empty($_POST['toranzahl']) && !empty($_POST['laufzeit'])) {
        $name = htmlspecialchars($_POST['name']);
        $toranzahl = htmlspecialchars($_POST['toranzahl']);
        $laufzeit = htmlspecialchars($_POST['laufzeit']);

        try {
            $stmt = $pdo->prepare("INSERT INTO trainings (benutzername, toranzahl, laufzeit, datum) VALUES (:benutzername, :toranzahl, :laufzeit, NOW())");
            ...
            $message = "<div class='text-red-600 font-semibold mt-4'>Fehler beim Speichern des Highscores.</div>";
        }
    } else {
        ...
        <section class="snow-effect max-w-lg w-full text-center">
            ...
            <form method="POST" action="trainer.php" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="number" name="laufzeit" id="laufzeit" min="1" step="0.01" required 
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
